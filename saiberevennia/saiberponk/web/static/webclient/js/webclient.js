 ...
 *
 * regex_funcs plugin
 * enables calling a plugin function based on regex matching some text sent from the server.
 *
 */
let tooltipCatcher = (function () {
    var tooltip = function (arg, kwargs, key) {
        regex = new RegExp( key, 'g' ); 
        return arg.replace( regex, "<span class='tooltip' data-tooltip=\"" + regexFuncs[key][1] + "\">$1</span>" );
    };
    var onclick = function (arg, kwargs, key) {
        regex = new RegExp( key, 'g' ); 
        return arg.replace( regex, "<span onClick=\"" + regexFuncs[key][1] +"\">$1</span>" );
    };
    var simple_replace = function (arg, kwargs, key) {
        regex = new RegExp( key, 'g' ); 
        return arg.replace( regex, "$1" );
    };
 
    let regexFuncs = {
        // modify args in place to add an intro css tooltip
        "([Cc]harcudoc[s]?)": [ tooltip, "Charcudoc (n.m) - Un chirurgien spécialisé dans l'implantation de matériel cybernétique illégal/non-enregistré." ],
		"([Mm]atrice)": [ tooltip, "Matrice (n.f) - Réseau d'intercommunication global, à disposition des habitants de la ville." ],
		"([Tt]ridéo)": [ tooltip, "Tridéo (n.f) - Successeur tridimensionnel de la vidéo ; abréviation : « trid »" ],

         // add other regex functions here
    };
 
    let kregexes = Object.keys( regexFuncs ); // array of regular expression strings
    let regexes  = kregexes.map( function (key) { return new RegExp(key, 'g') } ); // array of regular expression objects
 
    //
    // for each regex/function pair in regexFuncs:
    //     run through the received message and modify in place the content of it.
    //     then return the modified message
    var onText = function (args, kwargs) {
		console.log("ontext",args,kwargs);
		let modifyMessage = args
		// Go through all the regexps
        for( let x=0; x<regexes.length; x++ ) {
			// Use the regexp to replace with the appropriate content
			modifyMessage = regexFuncs[ kregexes[x] ][0]( modifyMessage, kwargs, kregexes[x] )
        }
		console.log("endtext",modifyMessage);
		...
		// it would look like this: obj.msg(text=("a message", {'type': 'map'});
			...
			switch(msg[2].type) {
				...
				case 'dmg':
					SendFunc('get_status', '', {});
				default:
					message = tooltipCatcher.onText(message)
					GameMsg(message);
			}
		}
	}
